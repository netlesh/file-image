  ...
  <div class="image">
    ...
    >
      <div v-if="imageError" class="image-error">
        <v-icon
          large
          :name="imageError === 'UNKNOWN' ? 'error_outline' : 'info_outline'"
        />
...
import api from "directus/api";
import formatFilesize from "directus/app/src/utils/format-filesize";
import FileLightbox from "directus/app/src/views/private/components/file-lightbox";
import { nanoid } from "nanoid";
import { getRootPath } from "directus/app/src/utils/get-root-path";
import { unexpectedError } from "directus/app/src/utils/unexpected-error";
import { addTokenToURL } from "directus/api";
import DrawerItem from "directus/app/src/views/private/components/drawer-item";
  ...
  setup(props, { emit }) {
    ...
    const src = computed(() => {
      ...
      if (image.value.type.includes("image")) {
        const url =
          getRootPath() +
          `assets/${image.value.id}?key=system-large-cover&cache-buster=${cacheBuster.value}`;
    ...
    const meta = computed(() => {
      ...
      if (width && height) {
        return `${n(width)}x${n(height)} • ${formatFilesize(
          filesize
        )} • ${type}`;
      }
    ...
    async function fetchImage() {
      ...
      try {
        const id =
          typeof props.value === "string" ? props.value : props.value?.id;
